...
public class AdventOfCode3_2 {
    ...
    static String input = "..........................................389.314.................206......................449.523..................138.....................\n" +
            ".........+.....954......723..........................................*.............687.....*..........692..........*........................\n" +
    ...
    public static void main(String[] args) {
        List<char[]> inputArr = new ArrayList<>(Arrays.stream(input.split("\n")).map(String::toCharArray).toList());
        long rs = 0;
        for (int i = 0; i < inputArr.size(); i++) {
            for (int j = 0; j < inputArr.get(i).length; j++) {
                if (inputArr.get(i)[j] == '*') {
                    rs += findNumberAround(i, j, inputArr);
                }
            }
        }
    ...
    public static long findNumberAround(int x, int y, List<char[]> arr) {
        int firstRow = x == 0 ? x : x - 1;
        int lastRow = x == arr.size() - 1 ? x : x + 1;
        int firstCol = y == 0 ? y : y - 1;
        int lastCol = y == arr.get(y).length - 1 ? y : y + 1;
        int n1 = 0;
        int n2 = 0;
        for (int i = firstRow; i <= lastRow; i++) {
            for (int j = firstCol; j <= lastCol; j++) {
                if (isNumber(arr.get(i)[j])) {
                    ImmutablePair<Integer, Integer> pair = numberPossible(j, arr.get(i));
                    j = pair.getRight() + 1;
                    if (n1 == 0) {
                        n1 = getNumber(pair.getLeft(), pair.getRight(), arr.get(i));
                    } else {
                        n2 = getNumber(pair.getLeft(), pair.getRight(), arr.get(i));
                    }
                }
            }
        }
    ...
    public static int getNumber(int x, int y, char[] row) {
    ...
    public static ImmutablePair<Integer, Integer> numberPossible(int x, char[] row) {
        int xS = x;
        for (int i = x; i >= 0; i--) {
            if (i == x) continue;
            if (!isNumber(row[i])) {
        ...
        ImmutablePair<Integer, Integer> pair = new ImmutablePair<>(xS, x);
        for (int i = x + 1; i < row.length; i++) {
            if (!isNumber(row[i])) {
                break;
            }
            pair = new ImmutablePair<>(xS, i);
        }
        return pair;
    }
...
//                if (isNumber(row[j])) {
//                    int num = getNumberPossible(j, row);
//                    String  xy = isAdjacentSymbol(i, j, j - 1 + Integer.toString(num).length() ,inputArr);
//                    if (xy != null){
//                        set.add(i + "_" + j + "_" + (j - 1 + Integer.toString(num).length()) + "@" + xy);
//                        j = j + Integer.toString(num).length();
//                    }
//                }
//            }
//        }
//
//        for (String s: set.stream().toList()) {
//            String xySymBol = s.split("@")[1];
//            int xSymbol = Integer.parseInt( xySymBol.split("_")[0]);
//            int ySymbol = Integer.parseInt( xySymBol.split("_")[1]);
//            String xyFirstNum = s.split("@")[0];
//            int xNum = Integer.parseInt( xyFirstNum.split("_")[0]);
//            int yNum = Integer.parseInt( xyFirstNum.split("_")[1]);
//            int nextNum = findNumberAroundSymbol(xSymbol, ySymbol, xNum, yNum, inputArr);
//            if (nextNum != 0) {
//                rs+= nextNum *
//            }
//        }
...
//    }
    ...
    public static int findNumberAroundSymbol(int x, int y, int xEcl, int yEcl, List<char[]> inputArr) {
        int firstRow = x == 0 ? x : x - 1;
        int lastRow = x == inputArr.size() - 1 ? x : x + 1;
        int firstCol = y == 0 ? y : y - 1;
        int lastCol = y == inputArr.get(y).length - 1 ? y : y + 1;
        ...
        for (int i = firstRow; i <= lastRow; i++) {
            for (int j = firstCol; j <= lastCol; j++) {
                ...
                if (isNumber(inputArr.get(i)[j])) {
                    return getNumberPossible(j, inputArr.get(i));
                }
            }
        }
        return 0;
    ...
    public static String isAdjacentSymbol(int rowNum, int from, int to, List<char[]> inputArr) {
        int firstRow = rowNum == 0 ? rowNum : rowNum - 1;
        int lastRow = rowNum == inputArr.size() - 1 ? rowNum : rowNum + 1;
        int firstCol = from == 0 ? from : from - 1;
        int lastCol = to == inputArr.get(rowNum).length - 1 ? to : to + 1;

        for (int i = firstRow; i <= lastRow; i++) {
            for (int j = firstCol; j <= lastCol; j++) {
                char c = inputArr.get(i)[j];
                if (c == '*') {
                    return i + "_" + j;
                }
            }
    ...
    public static int getNumberPossible(int from, char[] row) {
        StringBuilder s = new StringBuilder();
        s.append(row[from]);
        for (int i = from + 1; i < row.length; i++) {
            if (isNumber(row[i])) {
                s.append(row[i]);
            } else {
                break;
