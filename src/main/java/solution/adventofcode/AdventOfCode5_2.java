...
public class AdventOfCode5_2 {


    static String[] inputSeed = ("2494933545 159314859 4045092792 172620202 928898138 554061882 2740120981 81327018 2031777983" +
            " 63513119 2871914181 270575980 2200250633 216481794 3289604059 25147787 3472625834 10030240" +
            " 260990830 232636388").split(" ");

    static String seed_to_soil = """
            ...
            """;
    static String soil_to_fertilizer = """
            ...
            """;
    static String fertilizer_to_water = """
    ...
    static String water_to_light = """
            ...
            """;
    static String light_to_temperature = """
            ...
            """;
    static String temperature_to_humidity = """
            ...
            """;
    static String humidity_to_location = """
    ...
    public static void main(String[] args) {
        // range lc -> hu
        Map<Long, String> mapSs = new TreeMap<>();
        Map<Long, String> mapSf = new TreeMap<>();
        Map<Long, String> mapFw = new TreeMap<>();
        Map<Long, String> mapWl = new TreeMap<>();
        Map<Long, String> mapLt = new TreeMap<>();
        Map<Long, String> mapTh = new TreeMap<>();
        Map<Long, String> mapHl = new TreeMap<>();
        for (String s : seed_to_soil.split("\n")) {
            mapSs.put(Long.parseLong(s.split(" ")[1]), s);
        }
        for (String s : soil_to_fertilizer.split("\n")) {
            mapSf.put(Long.parseLong(s.split(" ")[1]), s);
        }
        for (String s : fertilizer_to_water.split("\n")) {
            mapFw.put(Long.parseLong(s.split(" ")[1]), s);
        }
        for (String s : water_to_light.split("\n")) {
            mapWl.put(Long.parseLong(s.split(" ")[1]), s);
        }
        for (String s : light_to_temperature.split("\n")) {
            mapLt.put(Long.parseLong(s.split(" ")[1]), s);
        }
        for (String s : temperature_to_humidity.split("\n")) {
            mapTh.put(Long.parseLong(s.split(" ")[1]), s);
        }
        for (String s : humidity_to_location.split("\n")) {
            mapHl.put(Long.parseLong(s.split(" ")[1]), s);
        }
        List<long[]> rs = new ArrayList<>();
        for (int i = 0; i < inputSeed.length; i = i + 2) {
            List<long[]> rs1 = rangePossible(Long.parseLong(inputSeed[i]),
                    Long.parseLong(inputSeed[i]) + Long.parseLong(inputSeed[i + 1]) - 1, mapSs);
            for (long[] l1 : rs1) {
                List<long[]> rs2 = rangePossible(l1[0], l1[1], mapSf);
                for (long[] l2 : rs2) {
                    List<long[]> rs3 = rangePossible(l2[0], l2[1], mapFw);
                    for (long[] l3 : rs3) {
                        List<long[]> rs4 = rangePossible(l3[0], l3[1], mapWl);
                        for (long[] l4 : rs4) {
                            List<long[]> rs5 = rangePossible(l4[0], l4[1], mapLt);
                            for (long[] l5 : rs5) {
                                List<long[]> rs6 = rangePossible(l5[0], l5[1], mapTh);
                                for (long[] l6 : rs6) {
                                    rs.addAll(rangePossible(l6[0], l6[1], mapHl));
                                }
                            }
                        }
                    }
    ...
    private static long[] rowToLong(String s) {
        String[] tmp = s.split(" ");
        long[] rs = new long[3];
        rs[0] = Long.parseLong(tmp[0]);
        rs[1] = Long.parseLong(tmp[1]);
        rs[2] = Long.parseLong(tmp[2]);
        return rs;
    }

    public static List<long[]> rangePossible(long from, long to, Map<Long, String> map) {
        List<long[]> rs = new ArrayList<>();
        long[] row;
        int currentRow = 0;
        for (Map.Entry<Long, String> s : map.entrySet()) {
            row = rowToLong(s.getValue());
            ...
            if (from < src) {
                if (to >= src) {
                    ...
                    tmp[1] = src - 1;
                    rs.add(tmp);
                    System.out.println("1. " + "k:" + s.getKey() + "f:" + from + " t:" + to + " -> " + tmp[0] + " " + tmp[1]);
                    from = src;
                    rs.addAll(rangePossible(from, to, map));
                    ...
                    tmp[1] = to;
                    rs.add(tmp);
                    System.out.println("2. " + "k:" + s.getKey() + "f:" + from + " t:" + to + " -> " + tmp[0] + " " + tmp[1]);
                    break;
                }
            }
            if (from < src + range) {
                ...
                if (to >= src + range) {
                    ...
                    tmp[1] = des + range - 1;
                    rs.add(tmp);
                    System.out.println("3. " + "k:" + s.getKey() + " f:" + from + " t:" + to + " -> " + tmp[0] + " " + tmp[1]);
                    from = src + range;
                    rs.addAll(rangePossible(from, to, map));
                    break;
                }
            ...
            if (currentRow == map.size()) {
                long[] tmp = new long[2];
                tmp[0] = from;
                tmp[1] = to;
                rs.add(tmp);
                System.out.println("4. " + "k:" + s.getKey() + "f:" + from + " t:" + to + " -> " + tmp[0] + " " + tmp[1]);
            }
